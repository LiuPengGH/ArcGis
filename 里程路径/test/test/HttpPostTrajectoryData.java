...
public class HttpPostTrajectoryData {

    private static final Logger logger = LoggerFactory.getLogger(HttpPostTrajectoryData.class);

    public static void postTrajectoryData(ResultSet resultS, String format, String phone,String dateNew) throws SQLException, ParseException {

        String day_length_date = resultS.getString("date");
        String sql;
        if (day_length_date == null || !day_length_date.substring(0, 10).equals(format)) {


             sql= "SELECT * FROM day_length_path_type where mobile_phone = '"+phone+"' and start_date  between '"+format+"08:30:00" +"' and " +"'"+dateNew+"'";


        } else {
            sql = "SELECT * FROM day_length_path_type where mobile_phone = '"+phone+"' and start_date between '"+day_length_date+"' and " +"'"+dateNew+"'";
        ...
        ResultSet resultSet = MySQLHelper.GetSqlVal(sql);
